...
def pretrain(run_dir,
  ...
  with tf.Graph().as_default():
    with tf.device(tf.train.replica_device_setter(
      ...
      temp_teacher = config.model
      temp_teacher.build(config.hparams, 
                        config.data_converter.output_depth, 
                        is_training=False)

      eval_op = temp_teacher.eval(
      ...
      teacher_var_dict = {v.name[:-2]:v for v in teacher_variables_to_restore}
      ...
      with tf.Session() as sess:
        ...
        for i, o, c, sl in tf.data.make_one_shot_iterator(dataset_fn().take(-1)):
          loss = temp_teacher._compute_model_loss(i, o, c, sl)['loss']
          print("LOSS ==", loss)
          teacher_losses.append(tuple(i, loss)) 
