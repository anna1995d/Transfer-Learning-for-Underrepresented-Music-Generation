...
def evaluate(run_dir,
  ...
  with tf.Graph().as_default():
    ...
    names_to_values, names_to_updates = model.eval(**_get_input_tensors(dataset, config))
    ...
    metric_values = tf_slim.evaluation.evaluate_once(
      logdir=eval_dir,
      master=master,
    ...
    results = dict()
    for k,v in zip(names_to_values.keys(), metric_values):
      results[k] = v
    return results
...
def run(
  ...
  if not run_dir:
    raise ValueError('Invalid run directory: %s' % run_dir)
  ...
  config_update_map = {}
  if examples_path:
    config_update_map['%s_examples_path' % mode] = os.path.expanduser(examples_path)
  if tfds_name:
    if examples_path:
      raise ValueError(
          'At most one of --examples_path and --tfds_name can be set.')
    config_update_map['tfds_name'] = tfds_name
  ...
  if num_sync_workers:
    config.hparams.batch_size //= num_sync_workers
  ...
  if finetune and not is_training:
    raise ValueError('Invalid value. Cannot finetune in eval mode: {}'.format(mode))

  if finetune:
    if trainable_vars.lower() == 'all':
      trainable_vars = 'all'
    elif trainable_vars.lower() == 'last_layer':
      trainable_vars = ['decoder/output_projection/kernel:0', 'decoder/output_projection/bias:0']
    else:
      trainable_vars = _parse_var_list(trainable_vars)
  ...
  if is_training:
    ...
    num_batches = eval_num_batches or data.count_examples(
        config.eval_examples_path,
        config.tfds_name,
        config.data_converter,
        file_reader) // config.hparams.batch_size
    tf.logging.info("=======================================")
    tf.logging.info("Batch size = ", config.hparams.batch_size)
    tf.logging.info("num_batches = ", num_batches)
    tf.logging.info("=======================================")
    eval_dir = os.path.join(run_dir, 'eval' + eval_dir_suffix)
    result = evaluate(
        run_dir,
        eval_dir,
        config=config,
