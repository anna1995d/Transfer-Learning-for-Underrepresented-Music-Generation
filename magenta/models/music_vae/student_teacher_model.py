...
class StudentTeacher(object):
  ...
  def __init__(self): 
    self._teacher = base_model.MusicVAE(lstm_models.BidirectionalLstmEncoder(),
                                        lstm_models.CategoricalLstmDecoder(),
                                        name_or_scope = 'teacher')
  ...
  def build(self, hparams, output_depth, is_training):
    ...
    self._teacher.build(hparams,
                        output_depth,
                        is_training=False) # teacher is loaded and not trained
  ...
  def _compute_model_loss(
    ...
    student_loss = self._student._compute_model_loss(input_sequence,
                                                     ...
                                                     sequence_length,
                                                     control_sequence)[1]
    teacher_loss = self._teacher._compute_model_loss(input_sequence,
                                                     output_sequence,
                                                     sequence_length,
                                                     control_sequence)[1]
